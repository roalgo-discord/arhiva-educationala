...
void update(int Number, int Node, int Left, int Right, int Position, long long Value) {
    if (Left == Right) {
    ...
    segtree[Number][Node] = segtree[Number][Node * 2] + segtree[Number][Node * 2 + 1];
}
long long query(int Number, int Node, int Left, int Right, int L, int R) {
    ...
    int mid = (Left + Right) / 2;
    return query(Number, Node * 2, Left, mid, L, R) + query(Number, Node * 2 + 1, mid + 1, Right, L, R);
}
int main() {
