...
<!-- Demonstratie scurta -->
<!-- Ne putem baza pe același principiu ca la `DSU`, anume că dacă la un moment dat avem un element $x$ într-un set de mărime $s$, în cel mai rău caz, acesta va fi reunit cu un set tot de mărime $s$. Astfel, următorul set în care se va afla elementul va avea cel puțin $2s$ elemente, sau altfel spus, orice element va fi inserat în maxim $log(n)$ seturi pe parcursul algoritmului. Așadar, vom avea complexitate $O(nlog^2(n))$, deoarece și inserările iau $O(log(n))$. -->
