...
<Callout type="def" title="Ce este o stivă?">
...
<Callout type="note" title="Observație">
...
### Problema stack
<ProblemPreview url="https://kilonova.ro/problems/2001" />
...
### Problema stack\_max\_min
<ProblemPreview url="https://kilonova.ro/problems/2107" />
...
### Problema skyline
<ProblemPreview url="https://nerdarena.ro/problema/skyline" /> 
...
### Problema unific
<ProblemPreview url="https://kilonova.ro/problems/835" />
...
### Problema swap
<ProblemPreview url="https://kilonova.ro/problems/1076" />
...
2. $$S_i = \text{)}$$ și $$S_{i + 1} = ($$. În acest caz, există un $$a (a < i)$$ și un $$b (i  - 1 \< b)$$astfel încât$$S_a = \text{(}$$, $$S_b = \text{)}$$, și $$(a, i)$$, respectiv $$(i + 1, b)$$formau perechi. Costurile lor însumate vor fi$$i - a + b - i - 1 = b - a - 1$$. Când interschimbăm $$S_i$$cu $$S_{i + 1}$$obținem perechile$$(a, b)$$și$$(i, i + 1)$$, ale căror costuri însumate dau $$b - a + 1$$. Deci răspunsul a crescut cu 2, ceea ce înseamnă că nu este o operație swap validă.
3. $$S_i = \text{(}$$ și $$S_{i + 1} = \text{)}$$. Dacă nu există nici un $$a (a < i)$$ astfel încât $$S_a = \text{(}$$ și perechea lui $$a$$ (pe care o notăm cu $$b$$) să fie mai mare ca $$i   - 1$$, atunci operația nu ar fi validă, deoarece nu am avea pereche pentru $$S_i$$ dacă $$S_i = \text{)}$$. Mai întai, avem perechile $$(a, b)$$ și $$(i, i + 1)$$. După cum am văzut la cazul 2, răspunsul ar fi mai mic cu 2 dacă perechile ar fi $$(a, i)$$ și $$(i + 1, b)\$$. Deci operația swap este validă.

Observăm că singurul caz în care operația swap este validă este atunci când $$S_i = \text{(}, S_{i + 1} = \text{)}$$ și există un $$a < i$$, $$S_a = \text{(}$$, a cărui pereche $$b$$ este mai mare ca $$i + 1$$. Acest lucru se poate simplifica astfel: Căutăm un $$i$$ care respectă următoarele condiții:
...
<ProblemPreview url="https://codeforces.com/contest/1364/problem/C" />
