...
<Callout type="def">
  Într-un graf orientat și aciclic, definim sortarea topologică ca fiind o ordine a nodurilor (nu neapărat unică), astfel încât, dacă notăm cu $$P$$ lista pozițiilor nodurilor, iar $$A$$ și $$B$$ sunt două noduri, cu muchie de la $$A$$ la $$B$$, $$P\_A \< P\_B $$
</Callout>
...
Datorită faptului că folosim o singură parcurgere `DFS`, algoritmul va avea complexitate liniară $$\mathcal{O}(n + m)$$, unde $$n$$ este numărul de noduri, iar $$m$$ numărul de muchii. Memoria va fi tot $$\mathcal{O}(n + m)$$.
...
int main() {
    ...
    for (auto e : edges) {
        if (poz[e.first] > poz[e.second]) {
            // Am gasit o muchie pentru care nu este respectata ordinea
            // topologica
            std::cout << "IMPOSSIBLE";
            return 0;
        }
