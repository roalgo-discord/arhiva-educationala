...
Pentru a optimiza această abordare, vom precalcula costurile pentru toate numerele de la 1 la $$10^6$$. Pentru aceasta, vom utiliza ciurul lui Eratostene, pentru a genera numerele prime $$\leq 10^6$$, urmând ca mai apoi costul să fie calculat în O(1) pentru fiecare număr. Complexitatea precalculării este O($$n
\log \log n$$).
