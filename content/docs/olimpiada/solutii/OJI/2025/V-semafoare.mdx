...
Pentru cazurile în care $T_1 + T_2 > 0$ (cel puţin unul dintre cele două semafoare nu pornește la momentul curent), observăm că fiecare semafor funcţionează pe baza unui ciclu temporar ce se repetă continuu. O soluţie posibilă constă în parcurgerea următorilor pași:

- calculăm durata ciclului pentru fiecare semafor: $c_1 = R_1 + G_1 + V_1 + G_1$ și $c_2 = R_2 + G_2 + V_2 + G_2$
...
- determinăm timpul $tv_2$ până când se aprinde verde la cel de-al doilea semafor (similar)
