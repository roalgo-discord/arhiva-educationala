...
Dacă definim distanța între două vârfuri ale unui graf neorientat ca fiind lungimea celui mai scurt lanț dintre care are drept capete vârfurile, atunci putem să observăm că un vârf oarecare $$Z$$ se află pe un lanț de lungime minimă dintre $$X$$ și $$Y$$ dacă și numai dacă $$d(X,Z) + d(Z,Y) = d(X,Y)$$, pentru cazul în care considerăm lungimea lanțului ca fiind numărul muchiilor, și $$d(X,Z) +
d(Z,Y) = d(X,Y) + 1$$, pentru cazul în care considerăm lungimea ca fiind numărul vârfurilor.
...
Un vârf $$z$$ are proprietatea de a aparține tuturor drumurilor optime dintre $$X$$ și $$Y$$ dacă și numai dacă $$d(X,z) + d(Y,z) = d(X,Y)$$, iar $$nr(X,z) \cdot nr(Y,z)
= nr(X,Y)$$.
