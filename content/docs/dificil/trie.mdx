...
<Callout type="obs">
  Nu există două cuvinte, unul provenit din cele $$N$$ date și celălalt ($$s_k$$) din primele $$i-1$$ astfel încât ambele să fie prefixe pentru $$s_i$$. Dacă ambele sunt prefixe pentru $$s_i$$, atunci fie $$s_k$$ este prefix pentru un cuvânt din cele $$N$$, fie invers.
</Callout>
...
Cu alte cuvinte, o soluție în $$\mathcal{O}(M^2 + M \cdot S)$$ este posibilă, unde $$S = \sum_{i=1}^{N} len(i)$$. Putem optimiza soluția, observând că de fiecare dată putem face tranzițiile în $$\mathcal{O}(1)$$. Soluția finală devine $$\mathcal{O}(M + S)$$ sau $$\mathcal{O}(M \cdot \log + S)$$.
