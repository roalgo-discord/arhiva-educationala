...
În mod similar, pentru a defini $$dp\[1]\[i]$$, vom vrea să cuplăm nodul $$i$$ cu unul din fiii săi, iar mai apoi să preluăm toate maximele de la celelalte noduri. Astfel, $$dp\[1]\[i]$$ va fi egal cu $$\max (1 + dp\[0]\[x]+ \sum \max(dp\[0]\[x\_0],
dp\[1]\[x\_0]))$$, unde $$x$$ este fiul ales, iar $$x\_0$$ reprezintă oricare din ceilalți fii ai nodului $$i$$.
