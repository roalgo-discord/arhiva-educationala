...
Deși acest articol poate fi parcurs fără cunoștințe anterioare, se recomandă parcurgerea [articolului anterior despre vectori](./arrays.md).
...
Se recomandă citirea în prealabil a [articolului următor despre șiruri de caractere](./strings.md).
...
De exemplu, dacă avem `#!cpp pair<pair<int, int>, pair<int, int>> p = {{2, 4}, {1, 3}};`, cele patru elemente vor putea fi declarate și accesate după cum urmează:
...
Deși pentru mai multe detalii, puteți accesa [articolul nostru despre cozi](../mediu/queue.md), mai jos puteți găsi un exemplu de folosire a acestor instrucțiuni.
...
Deși pentru mai multe detalii, puteți accesa [articolul nostru despre stive](../mediu/stack.md), mai jos puteți găsi un exemplu de folosire a acestor instrucțiuni.
...
Deși pentru mai multe detalii, puteți accesa [articolul nostru despre deques](../mediu/deque.md), mai jos puteți găsi un exemplu de folosire a acestor instrucțiuni.
...
În general, vom vrea să folosim o coadă de priorități atunci când vrem să aflăm mai rapid cel mai mare sau cel mai mic element, constanta fiind bună, fapt ce face această structură de date principala metodă de a implementa diverși algoritmi de tip greedy mai complicați, cel mai cunoscut fiind [algoritmul lui Dijkstra](../mediu/shortest-path.md#algoritmul-lui-dijkstra) pe grafuri cu costuri.
