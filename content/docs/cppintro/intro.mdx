...
De asemenea, folosim acolade pentru a grupa blocuri de cod. Acestea ajută la îmbunătățirea lizibilității programului și sunt necesare pentru a marca instrucțiunile subordonate structurilor alternative (de ex., `#!cpp if`) și repetitive (de ex., `#!cpp while`).
...
Funcția `#!cpp int main()` este esențială în orice program C++, fiind prima funcție apelată de compilator.
